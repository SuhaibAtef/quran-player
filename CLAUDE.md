...
Freshly scaffolded Flutter app (`flutter create`) intended to become a Quran player. [lib/main.dart](lib/main.dart) now hosts a minimal ForUI-based shell (`QuranPlayerApp` → `HomePage` with `FScaffold` / `FHeader` / `FButton`); the counter demo has been removed. [test/widget_test.dart](test/widget_test.dart) verifies the home page renders. Real features still need to be built on top.
...
- ForUI is wired via [`forui: ^0.17.0`](pubspec.yaml#L37). Theming uses `FThemes.zinc.light` with `toApproximateMaterialTheme()` on the underlying `MaterialApp`, plus a `FTheme` builder. `FLocalizations.localizationsDelegates` / `supportedLocales` are registered so ForUI widgets localize correctly.
