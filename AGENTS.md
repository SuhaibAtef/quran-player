...
Wired today (after `bootstrap-foundation`, `quran-data-layer`, `mushaf-reader`, `audio-player-foundation`, `basic-quran-search`, `add-tafsir-data`):
...
What's not yet implemented: bookmarks, MCP server, offline audio downloads, multiple reciters, repeat/speed controls, MCP playback control, tafsir UI / reader view, topical index (deferred — no public structured concordance available), and semantic search (tiers I/II/III). Each lands in its own OpenSpec change against this foundation. Semantic-search architecture is ratified up front in [`openspec/specs/semantic-search/spec.md`](openspec/specs/semantic-search/spec.md); the tier-1/2/3 implementations land separately and reference that spec.
