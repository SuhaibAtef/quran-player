...
Conventions: **state** via `flutter_riverpod` (providers next to the feature; cross-cutting in [lib/app/state/](lib/app/state/)). **Routing** via `go_router` with paths in `RoutePaths`, names in `RouteNames` ([lib/app/router/route_names.dart](lib/app/router/route_names.dart)); unknown paths → `/`; shell switches `FSidebar` (≥768) ↔ `FBottomNavigationBar` ([lib/app/widgets/app_shell.dart](lib/app/widgets/app_shell.dart)). **Errors**: `Result<T>` ([lib/core/error/result.dart](lib/core/error/result.dart)) + sealed `Failure` ([lib/core/error/failure.dart](lib/core/error/failure.dart)); throw only on programmer errors. **Logging**: `appLogger` ([lib/core/logging/logger.dart](lib/core/logging/logger.dart)) configured once in `main()` via `initLogging()`. Never `print`. **Localization**: every user-facing UI-chrome string resolves through `AppLocalizations` — ARB catalogues in [lib/l10n/](lib/l10n/) (`app_en.arb` is the template, `app_ar.arb` the Arabic translation), regenerated by `flutter gen-l10n`; the generated `app_localizations*.dart` is gitignored build output. Never hardcode chrome strings. Chrome layout uses direction-aware primitives (`EdgeInsetsDirectional`, `AlignmentDirectional`, `start`/`end`) so it mirrors under RTL; Quran/mushaf content stays pinned to `TextDirection.rtl` regardless of UI locale. Display numbers in chrome go through `localizedNumber` ([lib/core/l10n/display_number.dart](lib/core/l10n/display_number.dart)); identifiers, route params, and stored keys stay ASCII.
