...
User-writable storage: `path_provider.getApplicationSupportDirectory()/quran/user.db` (created on first MCP-enabled startup). Schema v1 has only `audit_log`. Future bookmark and playback-history changes share this DB and bump the schema. The bundled `quran.sqlite` and `muyassar.sqlite` remain read-only and fail-closed; `user.db` fail-soft.
