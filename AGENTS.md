...
- **Local MCP surface** — in-app `mcp_server` Streamable HTTP endpoint under [lib/data/mcp/](lib/data/mcp/) with framework-free lifecycle/error/playback contracts under [lib/domain/mcp/](lib/domain/mcp/). MCP Status starts/stops the loopback-only server, then shows the `https://localhost:<port>/mcp` URL and a fresh bearer token for local LLM/MCP clients. The visible HTTPS endpoint uses an ephemeral self-signed localhost certificate and proxies to the package-backed transport, which still enforces bearer-token auth. It exposes read-only Quran/reciter tools (`search_quran`, `get_ayah`, `get_surah`, `list_surahs`, `list_reciters`) and resources (`quran://metadata`, `quran://surahs`, `quran://surah/{surah}`, `quran://ayah/{surah}/{ayah}`, `quran://reciters`) through the same repositories as the app. Playback tools (`play_surah`, `play_ayah`, `pause_playback`, `resume_playback`, `stop_playback`, `set_repeat`) bridge into the running app player only after per-command approval in MCP Status; denied or timed-out commands do not touch player state. No non-loopback listener, arbitrary file access, shell execution, persistent audit log, tafsir, semantic search, or bookmarks over MCP yet.
...
lib/
  ...
  data/mcp/                # local MCP Streamable HTTP adapter, DTOs, validation, tool/resource service
...
| `just mcp-smoke` | focused `flutter test` files | MCP service, HTTPS Streamable HTTP auth, safety-boundary, playback bridge, and status UI smoke tests |
