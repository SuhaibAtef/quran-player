...
Currently configured project: `quran_player` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/quran_player/SETUP.md` to fill in
   `data/quran_player/INFO.md`.
...
`--project-id quran_player` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/quran_player/`.
