# Agent setup for `quran_player`

This is a deepsec scanning workspace. Project `quran_player` was just registered
(target: `..`). Setup is incomplete — `data/quran_player/INFO.md`
...
2. **Fill in `data/quran_player/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id quran_player
pnpm deepsec process --project-id quran_player
```
