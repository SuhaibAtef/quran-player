...
- Add a local-only MCP sidecar server for Quran data access and playback control.
...
- Add app-visible MCP status for server availability, local-only mode, read-only tools, playback-control permission state, and recent command decisions.
...
- New server-side Dart code for MCP protocol handling, tool/resource schemas, and JSON DTO mapping.
...
- Possible dependency addition for an MCP protocol package or a small in-repo stdio transport if no suitable Dart package meets the local-only/security constraints.
