...
The system SHALL record the user's last-read ayah position in `user.db`, updating it when the user leaves the reader. Exactly one reading position SHALL be retained — the most recent. The position SHALL be recorded in both page mode and text mode.
