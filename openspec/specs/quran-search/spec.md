# quran-search Specification

## Purpose
Basic user-facing Quran search: a Search top-level destination that accepts a plain Arabic text query, runs it against the bundled Quran corpus through `QuranRepository.searchAyahs`, renders trustworthy ayah-anchored results, and opens any result through the existing reader deep link so reader-mode routing stays centralized.
