...
## Purpose
Architectural capability covering how the app does meaning-based search across the bundled corpora. Pins NeoAraBERT_MSA hosted on GitHub Release, lazy ONNX inference via flutter_onnxruntime, pure-Dart linear cosine scan over per-tier .bin files, three independent Settings toggles (semantic / topics / tafsir), sectioned meaning-mode UI, fail-closed per-tier failure modes that never disable keyword search, and the out-of-scope catalog (no fusion ranking, no persistent query cache, no sqlite-vec, no multilingual queries, no MCP exposure). Scenarios here are the acceptance bar that the tier-1, tier-2, and tier-3 implementation changes must satisfy.
