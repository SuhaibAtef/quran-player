# tafsir-data Specification

## Purpose
Canonical tafsir corpus on disk: a bundled, integrity-checked SQLite asset of al-Muyassar (King Fahd Complex) commentaries indexed by AyahKey, served through a framework-free `TafsirRepository` contract that cross-checks every key against the bundled Quran DB. Substrate for the future tafsir reader view and tier III semantic search.
