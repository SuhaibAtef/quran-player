...
## Purpose
User bookmarks: saving, listing, and removing individual ayahs, persisted across sessions in the user-writable `user.db`. Bookmarks are added and removed through a verse action menu in the reader and managed on the Bookmarks top-level destination, which opens any bookmark through the existing reader deep link.
